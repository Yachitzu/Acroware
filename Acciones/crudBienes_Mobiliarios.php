...
class AccionesBienes_mobiliarios
{
    ...
    public static function listarUsuariosOrigen()
    {
        try {
            ...
            $consulta = "SELECT DISTINCT usuarios.id as custodio, usuarios.nombre as nombre_usuario, usuarios.apellido as apellido_usuario 
                     FROM bienes_mobiliarios 
                     LEFT JOIN usuarios ON bienes_mobiliarios.custodio_actual = usuarios.id 
                     WHERE bienes_mobiliarios.activo = 'si' AND usuarios.id IS NOT NULL";
            $resultado = $conexion->prepare($consulta);
            $resultado->execute();
            $dato = $resultado->fetchAll(PDO::FETCH_ASSOC);
            $dato;
    ...
    public static function listarBienesPorCustodio($custodio_id)
    {
        try {
            $conexion = Conexion::getInstance()->getConexion();
            $consulta = "SELECT * FROM bienes_mobiliarios WHERE custodio_actual = :custodio_id AND activo = 'si'";
            $resultado = $conexion->prepare($consulta);
    ...
    public static function actualizarCustodioBienes($bienes, $custodioDestino)
    {
        try {
            ...
            $consulta = "UPDATE bienes_mobiliarios SET custodio_actual = :custodioDestino WHERE id = :id";
