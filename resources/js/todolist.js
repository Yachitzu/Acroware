(function($) {
    ...
    $(function() {
      ...
      var todoListUserId = $('.todo-list-input_id[name="usuario_id"]').val();
  
      // Agregar un nuevo recordatorio
      $('.todo-list-add-btn').on("click", function(event) {
        event.preventDefault();
        var item = todoListInput.val(); // Obtiene el valor del input de texto
        var userId = todoListUserId;
        if (item) {
          $.post('http://localhost/Acroware/Acciones/recordatorios/agregar_recordatorio.php', { actividad: item, usuario_id: userId }, function(data) {
            ...
            if (response.id) {
              ...
              todoListInput.val(""); // Limpia el input de texto
